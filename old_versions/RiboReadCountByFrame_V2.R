...
# extract orf_id
gtf_selected$V9 = gsub("\"; transcript_id","",str_extract(gtf_selected$V9,"(ENS[MUS]*T[0-9]+.*:chr.+; transcript_id)"))
...
#output 
cat("writing output ... ")
fwrite(result_f1_f2_f3, opt$out, sep="\t", quote = FALSE, row.names= FALSE)
t=second_to_humanReadableTime(round((proc.time() - ptm)[3],2)) 
cat(paste0(t[1],"h-",t[2],"m-",round(t[3]),"s\n"))
